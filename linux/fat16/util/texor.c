  ...
  while (i < row->rendered_size) {
    ...
    if (multiline_comment_start_length && multiline_comment_end_length &&
      ...
      if (in_comment) {
        ...
        if (!strncmp(&row->rendered_characters[i], multiline_comment_end,
      ...
      } else if (!strncmp(&row->rendered_characters[i], multiline_comment_start,
...
int editorRowFilePositionXToScreenPositionX(erow *row, int file_position_x) {
  ...
  for (j = 0; j < file_position_x; j++) {
    if (row->characters[j] == '\t')
      screen_position_x +=
          (TEXOR_TAB_STOP - 1) - (screen_position_x % TEXOR_TAB_STOP);
    screen_position_x++;
  }
  ...
  for (file_position_x = 0; file_position_x < row->size; file_position_x++) {
    if (row->characters[file_position_x] == '\t')
      cur_screen_position_x +=
          (TEXOR_TAB_STOP - 1) - (cur_screen_position_x % TEXOR_TAB_STOP);
    cur_screen_position_x++;
...
void editorUpdateRow(erow *row) {
  ...
  row->rendered_characters =
      malloc(row->size + tabs * (TEXOR_TAB_STOP - 1) + 1);
...
void editorInsertNewline() {
  if (E.file_position_x == 0) {
    ...
    editorInsertRow(E.file_position_y + 1, &row->characters[E.file_position_x],
...
void editorOpen(char *filename) {
  ...
  // if (!fp) die("fopen");
  int fp = fat_open(filename);
  if (fp == -1) die("fat_open");

  char *line = NULL;
  size_t linecap = 0;
  ssize_t linelen;
  while ((linelen = fat_getline(&line, &linecap, fp)) != -1) {
    while (linelen > 0 &&
           (line[linelen - 1] == '\n' || line[linelen - 1] == '\r'))
      linelen--;
...
void editorSave() {
  ...
  if (fd != -1) {
    ...
    // }
    // close(fd);
    if (fat_trunc(fd, len) != -1) {
      if (fat_write(fd, buf, len) == len) {
        fat_close(fd);
        free(buf);
        E.dirty = 0;
        editorSetStatusMessage("%d bytes written to disk", len);
    ...
    case CTRL_KEY('z'):
      if (E.dirty && quit_times > 0) {
        editorSetStatusMessage(
            ...
            "Press Ctrl-Z %d more times to quit.",
            quit_times);
        quit_times--;
        return;
      }
...
void openEditor(char *filename) {
  ...
  editorSetStatusMessage("HELP: Ctrl-S = save | Ctrl-Z = quit | Ctrl-F = find");
